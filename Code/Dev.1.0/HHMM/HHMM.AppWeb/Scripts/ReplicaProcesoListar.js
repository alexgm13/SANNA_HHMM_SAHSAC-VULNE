...
var cabecerasPrincipal = ["Descripción", "Fecha Inicio", "Fecha Fin", "Estado", "Usr.Creador", "Creación", "Usr.Modificador", "Modificación", "OA Ant.", "Exp Ant.", "OA Act.", "Exp Act.", "OA Ult.", "Exp Ult.",  ];
var matrizIndicePrincipal = [1, 2, 3, 4, 6, 7,11,12, 9, 10, 13, 14, 15, 16];
var anchosPrincipal = [200, 75, 75, 75, 100, 75, 100, 75, 60, 60, 60, 60, 60, 60];
var anchosExcelPrincipal = [250, 100, 100, 120, 200, 150, 200, 150, 130, 130, 130, 130, 130, 130];
...
var sucursal, sucursalId, isAdmin;
		...
		switch (identificador) {
			case "Principal":
				...
				for (; i > 0; i--) {
					...
					j = 0;
					nCampos = matrizFecha.length;
					for (; j < nCampos; j++) {
						matriz[x][matrizFecha[j]] = matriz[x][matrizFecha[j]].split(" ")[0];
					}
					j = 0;
					x++;
				}
...
function configurarControles() {
	...
	if (aAdicionar != null) {
		aAdicionar.onclick = function () {
			limpiarCampos();
			document.getElementById("TituloPopupPeriodo").innerHTML = "ADICIONAR REPLICA PROCESO";
	...
	hdfMedico.onchange = function () {
		if (this.value != "0") {
			...
			this.value = datos[0];
			document.getElementById("txtIMedico").value = datos[1];
		}
	}

	var btnIBuscar = document.getElementById("btnIBuscar");
	btnIBuscar.onclick = function () {
	    var lista = "";
	    objRespuestaProceso.dato1 = 0;
	    objRespuestaProceso.dato2 = 0;
	    objRespuestaProceso.dato3 = "";
		...
		var url = urlBase + "Mantenimiento/procesarReplicaProceso?ss=" + ss + "&sec=1";
		...
		for (var x = 0; x < desabiPop.length; x++) {
			desabiPop[x].style.pointerEvents = "none";
			desabiPop[x].disabled = false;
		}
		var btnIBuscar = document.getElementById("btnIBuscar");
		btnIBuscar.innerHTML = "<span class='Icons fa-refresh cargando'></span>";
	}
	...
	btngrabarEstado.onclick = function () {
		...
		var url = urlBase + "Mantenimiento/anularReplicaProceso/?ss=" + ss + "&id=" + id + "&est=" + (cboEstado == "A" ? "I" : "A") + "&anio=" + txtAnio + "&estado=" + cboEstado;
    ...
    btnBuscar.onclick = function () {
        ...
        var url = urlBase + "Mantenimiento/listarReplicaProceso?ss=" + ss + "&anio=" + (txtAnio + cboMes) + "&estado=" + cboEstado + "&sucursal=" + sucursalId;
	...
	btnIProcesar.onclick = function () {
		...
		var url = urlBase + "Mantenimiento/procesoInterfaseMaestro?ss=" + ss + "&entidad=" + cboEntidad;
	...
	aExportarExcel.onclick = function () {
		if (matrizPrincipal.length > 0) {
			...
			this.download = "ReplicaProceso.xls";
				...
				for (var i = inicio; i < fin; i++) {
					if (i < n) {
						...
						for (var x = 0; x < listaPeriodo.length; x++) {
							if (listaPeriodo[x][0] == matriz[i][8]) {
								encontrado = true;
								break;
							}
						}
						...
						if (matriz[i][4] == "A") {
							contenido += "<td style='text-align:center'>";
							if (encontrado) {
								contenido += "<a href='#' class='" + (matriz[i][4] == "I" ? "Icons fa-check" : "Icons fa-trash-o") + "' onclick='Campoeliminar=\"";
								contenido += i;
								contenido += "\";abrirPopup(\"PopupEstado\");' title='" + (matriz[i][4] == 'A' ? 'INACTIVO' : 'ACTIVO') + "'></a>&nbsp;";
...
function MostrarSeleccionado(cboDefault, valorDefault) {
    /* Para obtener el valor */
    document.getElementById(cboDefault).value = valorDefault;

    /* Para obtener el texto */
    //var combo = document.getElementById(cboDefault);
    //combo.options[combo.selectedIndex].text = sucursal;
}
	...
	if (rpta != "") {
		...
		if (datos[0] == "0") {
			...
			if (datos[4] == "4") {///->if (datos[3] == "6")
				...
				objRespuestaProceso.dato3 = (datos[0] == "1" ? "SI" : "NO");
				var hdfReplicaProcesoActivador = document.getElementById("hdfReplicaProcesoActivador").value;
				if (hdfReplicaProcesoActivador.toLowerCase() == "true") {
				    mostrarRespuestaAccion();
				}
				var btnIBuscar = document.getElementById("btnIBuscar");
				btnIBuscar.innerHTML = "Procesar";
				document.getElementById("btnBuscar").click();
				...
				var txtIOA = document.getElementById("txtIOA").value;
				lista += IdInterfaceActual + "¦" + txtIPeriodo + "¦" + cboISucursal + "¦" + hdfMedico + "¦" + (cboIEspecialidad == "" ? 0 : cboIEspecialidad) + "¦" + txtIOA;
				var url = urlBase + "Mantenimiento/procesarReplicaProceso?ss=" + ss + "&sec=" + datos[4];
				$.ajax(url, "post", listarBusquedaProceso, lista);
				mostraralerta(datos[1], true);
				switch (datos[4]) {
				    case "3":
						objRespuestaProceso.dato1 = datos[2] * 1;
						objRespuestaProceso.dato2 = datos[3] * 1;
				        break;
				    /*case "2":
...
function mostrarRespuestaAccion() {
    var spnReplicaResumen = document.getElementsByClassName("spnReplicaResumen");
    for (var x = 0; x < spnReplicaResumen.length; x++) {
        if (x == 0 || x == 1) {
            spnReplicaResumen[x].innerHTML = objRespuestaProceso.dato1;
        } else if (x == 2 || x == 3) {
            spnReplicaResumen[x].innerHTML = objRespuestaProceso.dato2;
        } else {
            spnReplicaResumen[x].innerHTML = objRespuestaProceso.dato3;
        }
    }
    abrirPopup('PopupReplicaProcesoMensaje');
}
...
function grabarPeriodo() {
	...
	var cboEstado = document.getElementById("cboEstado").value;
    if (opc == 1) {
        var url = urlBase + "Mantenimiento/GrabarReplicaProceso?ss=" + ss + "&anio=" + txtAnio + "&estado=" + cboEstado + "&opc=1" + "&sucursal=" + sucursalId;
		$.ajax(url, "post", mostrarGrabar, lista);
	}
	else {
		lista += "¦" + document.getElementById('hdfReplicaProcesoId').value;
        var url = urlBase + "Mantenimiento/GrabarReplicaProceso?ss=" + ss + "&anio=" + txtAnio + "&estado=" + cboEstado + "&opc=2" + "&sucursal=" + sucursalId;
		$.ajax(url, "post", mostrarGrabar, lista);
	}
}
...
function buscarReplicaProceso(id) {
	limpiarCampos();
	document.getElementById("TituloPopupPeriodo").innerHTML = "ACTUALIZAR REPLICA PROCESO";
	...
	for (var i = 0; i < n; i += 1) {
		if (option[i].value == matrizPrincipal[id][8]) {
			existe = true;
			break;
		}
	}
	if (existe) {
		cbo.value = matrizPrincipal[id][8];
	} else {
	...
	document.getElementById("txtEstado").value = (matrizPrincipal[id][4] == "A" ? "ACTIVO" : "INACTIVO");
...
function listarInterfacePopUp(id) {
	...
	window.top.validarPermisoParametro("Proceso de OAs y Expedientes").then(function (d) {

		if (d == "" || d == "1") {


			var limpiar = document.getElementsByName("limpiarPop");
			for (var y = 0; y < limpiar.leng1th; y++) {
			...
			document.getElementById("txtIPeriodo").value = matrizPrincipal[id][8];
			document.getElementById("hdfMedico").value = "0";
			//Codigo para mostar Sede por defecto y bloquear el combo si no es rol Administrador 14/10/2019
			MostrarSeleccionado("cboISucursal", sucursalId);
			document.getElementById("cboISucursal").disabled = (isAdmin == 1) ? false : true;
			document.getElementById("cboISucursal").style.backgroundColor = (isAdmin == 1) ? "white" : "#f4f4f4";
	...
	for (i = 0; i < nRegistros; i++) {
		...
		for (var j = 0; j < nCampos; j++) {
			...
			switch (j) {
                ...
                case 12:
                case 13:
					contenido.push(matrizPrincipal[i][matrizIndicePrincipal[j]]);
					break;
				default:
