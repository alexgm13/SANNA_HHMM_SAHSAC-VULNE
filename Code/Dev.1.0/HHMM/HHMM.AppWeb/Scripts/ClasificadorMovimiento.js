...
window.onload = function () {
    ...
    var url = urlBase + "Mantenimiento/listarClasificadorMovimiento/?ss=" + ss;
...
function listarClasificador(irUltimaPagina) {
    crearMatriz();
...
function crearTabla() {
    ...
    contenido += "</tr>";
    contenido += "</thead>";
    contenido += "<tbody id='tbClasificadorMovimiento' class='tabla-FilaCuerpo'>";
    contenido += "</tbody>";
    contenido += "<tfoot>";
    ...
    contenido += "</tfoot>";
    contenido += "</table>";
    document.getElementById("divClasificadorMovimiento").innerHTML = contenido;
}
...
function mostrarMatriz(indicePagina) {
    ...
    if (n > 0) {
        ...
        for (var i = inicio; i < fin; i++) {
            if (i < n) {
                ...
                contenido += "<td style='text-align:center'><input type='radio' name='rdnClasficadorMovimiento' id='rdnx";
                contenido += i;
    ...
    document.getElementById("tbClasificadorMovimiento").innerHTML = contenido;
    crearPaginas();
    configurarChecks();
}
...
function configurarControles() {
    var btnSeleccionarClasificador = document.getElementById("btnSeleccionarClasificador");
    btnSeleccionarClasificador.onclick = function () {
        if (idPasar != "") {
            var hdfCla = window.parent.document.getElementById("hdfCla");
            hdfCla.value = idPasar;
            hdfCla.onchange();
            window.parent.abrirPopup('PopupClasificador');
            limpiar();
        }
        else {
            return false;
    ...
    var btnCancelarClasificador = document.getElementById("btnCancelarClasificador");
    btnCancelarClasificador.onclick = function () {
        limpiar();
        window.parent.abrirPopup('PopupClasificador');
    }
}
...
function configurarChecks() {
    var rdnCentroCosto = document.getElementsByName("rdnClasficadorMovimiento");
    var nCampos = rdnCentroCosto.length;
    for (var x = 0; x < nCampos; x++) {
        rdnCentroCosto[x].onclick = function () {
...
function limpiar() {
    var rdnClasificadorMovimiento = document.getElementsByName("rdnClasificadorMovimiento");
    var nCampos = rdnClasificadorMovimiento.length;
    for (var x = 0; x < nCampos; x++) {
        if (rdnClasificadorMovimiento[x].checked) {
            rdnClasificadorMovimiento[x].checked = false;
            break;
        }
    }
    idPasar = "";
