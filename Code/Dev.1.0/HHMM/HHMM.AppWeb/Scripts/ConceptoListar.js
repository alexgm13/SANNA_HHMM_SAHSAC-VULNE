...
var matriz = [];
var matrizDetalle = [];
var cabeceras = ["Descripción", "Ind. Adm.", "Estado"];
var cabecerasDetalle = ["Código", "Descripción"];
var anchos = [35, 10, 15];
var anchosDetalle = [10, 30];
var matrizIndice = [1, 2, 3];
var matrizIndiceDetalle = [2,2];
...
window.onload = function () {
	...
	var url = urlBase + "Mantenimiento/listarConcepto?ss=" + ss + "&sucursal=" + sucursalId;
...
function crearTabla(detalle) {
	if (!detalle) {
		...
		contenido += "<th style='width:5%;vertical-align: middle;text-align: center;'><span class='Icons fa-plus' onclick='EscogerOpcion(true,true);limpiarFormulario(true);abrirPopup(\"PopupConcepto\");'></span></th>";
		for (var j = 0; j < nCampos; j++) {
			...
			if (j == 2) {
				...
				contenido += "<select class='Combo' name='cabecera' style='width:90%'><option value=''>TODOS</option><option value='True'>SÍ</option><option value='False'>NO</option></select>";
			}
			else {
...
function configurarControles() {
	...
	tbConcepto.onclick = function (e) {
		...
		if (cnt.nodeName == "TD") {
			...
			document.getElementById("spnDetalleConcepto").innerHTML = sanitizeHTML(tr.cells[1].textContent);
	...
	btngrabarDetalle.onclick = function () {
		if (validarConcepto(false)) {
			...
			var strDatos = hdfOpc + "|" + idConcepto + "|" + ddlServicioDetalle;
			if (hdfOpc == 2) {
				strDatos += "|" + idConceptoDetalle;
			}
			var ss = window.parent.document.getElementById("iss").value;
			var url = urlBase + "Mantenimiento/grabarConceptoDetalle?ss=" + ss + "&su=" + sucursalId;
			$.ajax(url, "post", mostrarGrabar, strDatos);
			abrirPopup("PopupConceptoDetalle");
		}
	}
