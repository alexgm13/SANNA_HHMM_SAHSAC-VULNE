...
window.onload = function () {
    ...
    var url = urlBase + "Mantenimiento/listarCentroCosto/?ss=" + ss;
...
function listarCentro(irUltimaPagina) {
    crearMatriz();
...
function crearTabla() {
    ...
    contenido += "</tr>";
    contenido += "</thead>";
    contenido += "<tbody id='tbCentroCosto' class='tabla-FilaCuerpo'>";
    contenido += "</tbody>";
    contenido += "<tfoot>";
    ...
    contenido += "</tfoot>";
    contenido += "</table>";
    document.getElementById("divCentroCosto").innerHTML = contenido;
}
...
function mostrarMatriz(indicePagina) {
    ...
    if (n > 0) {
        ...
        for (var i = inicio; i < fin; i++) {
            if (i < n) {
                ...
                contenido += "<td style='text-align:center'><input type='radio' name='rdnCentroCosto' id='rdnx";
                contenido += i;
    ...
    document.getElementById("tbCentroCosto").innerHTML = contenido;
    crearPaginas();
    configurarChecks();
}
...
function configurarControles() {
    var btnSeleccionarCentro = document.getElementById("btnSeleccionarCentro");
    btnSeleccionarCentro.onclick = function () {
        if (idPasar != "") {
            var hdfCen = window.parent.document.getElementById("hdfCen");
            hdfCen.value = idPasar;
            hdfCen.onchange();
            window.parent.abrirPopup('PopupCentro');
            limpiar();
        }
        else {
            return false;
    ...
    var btnCancelarCentro = document.getElementById("btnCancelarCentro");
    btnCancelarCentro.onclick = function () {
        limpiar();
        window.parent.abrirPopup('PopupCentro');
    }
}
...
function configurarChecks() {
    var rdnCentroCosto = document.getElementsByName("rdnCentroCosto");
...
function limpiar() {
    ...
    var nCampos = rdnCentroCosto.length;
    for (var x = 0; x < nCampos; x++) {
        if (rdnCentroCosto[x].checked) {
            rdnCentroCosto[x].checked = false;
            break;
        }
    }
    idPasar = "";
